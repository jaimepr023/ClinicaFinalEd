...
public class OperacionesImplementacion implements OperacionesInterfaz {
	...
	private PacientesDtos nuevoPaciente(List<EsclavoDto> listaEsclavo) {
		...
		if(afirmacion.equals("s")) {
			...
			condicion(listaEsclavo,pacienteN);
	
		}else {
			System.out.println("Se le redirigira a una nueva pagina para crear un nuevo tutor.");
	...
	private void condicion(List<EsclavoDto> listaEsclavo,PacientesDtos pacienteN) {
		for (EsclavoDto nom :  listaEsclavo) {
			if(pacienteN.getIdExclavo()!=nom.getIdEsclaco()) {	
